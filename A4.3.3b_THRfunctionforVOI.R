...
model.THR.voi <- function(age=60, male=0) {
  ...
  ####ANALYSIS####
  CE.output <- c(cost.SP0 = disc.cost.SP0,
  ...
  # 4 state costs
  # 5 transitions
  EVPI.data <- list(CE.output=CE.output, utilities= state.utilities,
                    costs=state.costs)
  # need to add the others, but just to show the idea
