...
i)	First, define the prevalence (30%) and a willingness to pay threshold (£30,000). 
...
iv) Now that you have calculated the NMB for each outcome, this can be used to estimate  the NMB for a strategy of 'treat all' or 'treat none'. Based on the parameters table, create a data frame comparing the costs, QALYs and NMB associated with the two strategies. Note that these NMB calculations are specific to the prevalence of the condition. You can define this data frame as the expected.values table, and the costs have been calcualted for you. Calculate the QALYs using the same calculation as the costs, and then calcualte the NMB, which has been left blank. 
...
vii) Now that you have run the function with different prevalence values, create a vector of prevalence values, from 0% to 100% (or 0 to 1) by 5% increments, you can call this 'prevalence.vector'. Pass the vector of prevalence values to the `est.nmb()` function.  Save the NMB values, the output of the function, so that we can create a plot to help visualise this change.   

viii) Now we can create a simple plot in base R, showing the NMB for the two strategies (treat all vs. treat none). The code for a simple plot has been provided for you. 
...
iii)	Assuming a diagnostic threshold of 0 ensure you can identify the True Positives, True Negatives, False Positives and False Negatives as per the video presentation. __(JW - ????)__
