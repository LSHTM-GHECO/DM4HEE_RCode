...
i)	Plot the distributions on a chart and verify you get the same as those presented in the teaching session (you will need to set the prevalence to 30% (0.3) if you have changed the original prevalence). 
...
iv) We can now save the TPR and FPR, and create a Receiver Operating Characteristic (ROC) curve, using the base R plot. 
...
ii) We can now use the TP, FP, FN, and TN to calculate the expected NMB of the imperfect test at each diagnostic threshold. This will involve getting the NMB payoff parameters assigned in at the start of the exercise. Once these have 
