...
In the template file for this exercise you will find a new section labelled 'THR Model Function'. The aim of this function is wrap the main parts of the Markov model into a function, so that we can repeat and record the results of the probabilistic analysis constructed in Module 2. 
...
* Try running `model.THR()` to check it runs before moving to the next stage (remember that runs the model for a female population, with mean age of 60). This may still take a fair few seconds to run so be patient on first running. You should have similar results to the below (remember actual numbers will differ):
