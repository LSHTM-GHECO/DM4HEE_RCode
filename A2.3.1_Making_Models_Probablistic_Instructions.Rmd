...
```{r, include = FALSE}
source("A2.3.3_Making_Models_Probablistic_Solutions.R", local = knitr::knit_global())
```
...
A template for the exercise, ‘A2.3.2_Making_Models_Probablistic_Template.R’, is available, and builds upon the deterministic model developed in Module 1. Again, we use the 3 packages used in Module 1 - `data.table`, `tidyr` and `dplyr`. So please do run the `library()` functions at the beginning of the script before continuing. 
...
iv) Use this information to fit a constant transition probability for the annual re-revision rate parameter, using the same approach as for operative mortality above. Your beta value for re-revision risk should be equivalent to __`r b.rrr`__ .
...
v) You have now created a vector of five multivariate normal parameters that are correlated according to the estimated covariance matrix. See below (again, the actual numbers will be different given that Tz is based on z, which is from a random draw):
...
Refer to the solutions from the Module 1 and try to complete the definitions currently blank, trying to understand the process of each step.
