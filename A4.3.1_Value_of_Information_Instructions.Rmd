...
**Part 2**: We will be using the "A4.3.2b_Value_of_Information_Part 2_Template.R" and "A4.3.3b_Value_of_Information_Part 2_Solutions.R" template and solution files respectively. These files will also source a version of the model developed to perform the EVPPI analyses, called **XXXXXXX**. In this part of the exercise we will be:

1. Walking through how the model function can be adapted to allow for parameters to provided as arguments. 
2. Selecting the appropriate parameter values to perform EVPPI analyses for each parameter (or group of parameters)
2. Plotting EVPPI results across a range of willingness to pay thresholds.
...
```{r model run, eval = TRUE}
model.THR.voi(RR.NP1 = RR.vec[a], lambda = lambda.vec[b], gamma = gamma.vec[b], omr = omr[b,], 
...
(vi) Once you have done this, the NMB values for each outer loop should look similar to the following:

```{r evppi example, include = FALSE}
#evppi.df
```
