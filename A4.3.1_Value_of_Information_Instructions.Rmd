...
(i) First define the WTP threshold at £100,000 per QALY gained
(ii) Estimate the NMB for SP0 and NP1 for each run using columns of the `simulation.results` data.table and your defined WTP
(iii) Estimate the the mean value of maximum NMB for each run (`av.perfect`) and the maximum of the simulation mean NMB (`max.sim.mean`) values via using the `apply()` function. We've given some further hints on the template. 
...
(3)	**Calculating the effective population and population EVPI**

Since EVPI is calculated using net-benefit and net-benefit is a function of the unknown ceiling ratio it is important to estimate EVPI as a function of the ceiling ratio. We therefore need to wrap up what we've done previously into a function, and run it across different WTP values.

(i) Define a new `WTP.values` vector running from 0 to 100,000 in increments of 100.
(ii) Based on what we've done so far, fill in the blanks left in the function `est.EVPI.pop()`. 
