...
The basic structure of the model is given in the figure below **(To be added)**, which shows that the chronic disease process is structured as  such that patients can move to successively more serious disease states, but cannot recover.
...
i) First create an empty trace matrix of the right dimensions - we can call this 'trace.AZT' (L99). The data, number of rows, and number of columns have been left blank for you to add in. 
...
Once you have done this, you should have calculated total undiscounted costs of £```round(undisc.cost.AZT,2)``` and discounted costs of £```round(disc.cost.AZT,2)```. 
