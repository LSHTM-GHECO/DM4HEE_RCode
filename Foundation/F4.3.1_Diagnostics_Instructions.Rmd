...
The function will take the parameter values (defined at the start of the exercise) and the lambda value (i.e. the willingness to pay threshold) by default, as these are assigned when the function is written. The function also requires the prevalence of the condition to evaluate the NMB. 
...
ii) Complete the cost and QALY calculations, and add the calculation for NMB in the function, using the `est.nmb` function as a guide to help you. 

iii)	Run the function, using the 30% prevalence. How does this NMB compare to the previous function, and why? _(Hint: You can run the the function using the 30% value two ways, either as shown below, to get the same output)_
...
iv) Now run the function using the vector of prevalence values that you created eaerlier ('prevalence.vector'), to get the NMB of a perfect test, across different prevalences. Make sure to save this as a data.frame - you can save this as 'perfect.test.nmb'.


v) Next, compare the results generated in the 'perfect.test.nmb' to the results saved earlier in the 'no.test.nmb' data frame. You should be able to see that the NMB for the perfect test is higher than the treat all or treat none strategies (except when the prevalence is 0 or 1). Run the code to create a plot to observe the results generated. The code for the plot is provided for you, and should look like this:. 
...
vii) Finally, You can do calculate the EVPDI by simply subtracting the max NMB without a test from the NMB with a perfect test, and save this as 'evpdi'. When the EVPDI results are added to a data frame with the prevalence, this is what the results should look like:
...
i) First, create a vector of diagnostic values from -4 to 4, in increments of 0.25. Use the `seq()` function to do this. This can be saved as 'diagnostic.threshold'. 
...
** Short descriptio about using ggplot, and the reshape used can be added here (or in the reader) ** 
