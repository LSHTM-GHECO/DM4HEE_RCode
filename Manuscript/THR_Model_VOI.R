...
## CHECKING THAT THE MODEL WORKS CORRECTLY - USES THE DATA SAMPLED (AGE AND GENDER SPECIFIC)
i = 1
...
for(i in 1:sim.runs){
  ...
  psa.results[i,] <-   model.THR.voi(RR.vec[i], omr.df[i,],  tp.rrr.vec[i], 
                      survival.df[i,],c.revision.vec[i], 
                      state.utilities.df[i,]) 
}
...
for(i in 1:sim.runs){
  simulation.results[i,] <- model.THR.voi(RR.vec[i], omr.df[i,],  tp.rrr.vec[i], 
                                          survival.df[i,],c.revision.vec[i], 
                                          state.utilities.df[i,])  ## running the model 
}
...
# 
# 
# model.THR.voi(RR.vec[2], omr.df[2,],  tp.rrr.vec[2], 
